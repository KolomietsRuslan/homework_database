...
SELECT `users`.`surname`, `users`.`name`, `areas`.`name` FROM `users` JOIN `areas` ON `users`.`area_id` = `areas`.`id` ORDER BY `areas`.`name` LIMIT 4
3. Необходимо посчитать сколько новостей в каждой категории. Связь `news`.`category_id` = `categories`.`id`.
SELECT `categories`.`name`, COUNT(`news`.`id`) FROM `news` JOIN `categories` ON `news`.`category_id` = `categories`.`id` GROUP BY `categories`.`id`
